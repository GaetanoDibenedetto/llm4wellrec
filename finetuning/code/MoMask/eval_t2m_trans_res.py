...
if __name__ == '__main__':
    ...
    # eval_val_loader, dataset = get_dataset_motion_loader(dataset_opt_path, 32, 'original-test', device=opt.device)
    eval_val_loader, dataset = get_dataset_motion_loader(dataset_opt_path, 10, 'test', device=opt.device)
    # eval_val_loader, dataset = get_dataset_motion_loader(dataset_opt_path, 32, 're-training-test', device=opt.device)
    # print(f"Dataset length: {len(dataset)}")
    # print(f"Dataloader length: {len(eval_val_loader)}")
    ...
    for file in os.listdir(model_dir):
        ...
        for i in range(repeat_time):
            with torch.no_grad():
                best_fid,  Rprecision, best_matching, best_mm = \
